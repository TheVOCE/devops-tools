...
function mockStateFullWorkItemInStream(stream, workItem, comments = []) {
    ...
    if (comments.length > 0) {
        ...
        comments.forEach((comment) => stream.markdown(`\n> ${comment.body?.replaceAll("\n", "\n> ") + ""}\n`));
    }
    stream.button({
...
function buildWorkItemPrompt(workItem, userPrompt, comments = []) {
    const title = workItem.fields["System.Title"];
    ...
    if (comments.length > 0) {
        contextMessage += ` Do also regard the comments: ${comments.map((comment) => comment.body).join("\n\n")}`;
    }
...
//# sourceMappingURL=workItemHandlingIntegrationTest.js.map